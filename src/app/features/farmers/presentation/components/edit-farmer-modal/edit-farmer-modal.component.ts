import { Component, inject, input, output, signal, computed, HostListener, OnInit } from '@angular/core';
import { CommonModule } from '@angular/common';
import { ReactiveFormsModule, FormBuilder, Validators } from '@angular/forms';
import { FarmerStore } from '../../../store/farmer.store';
import { Farmer, CreateFarmerRequest } from '../../../domain/farmer.model';
...
@Component({
  selector: 'app-edit-farmer-modal',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  template: `
    <div class="backdrop" (click)="onBackdropClick($event)">
      <aside class="drawer" (click)="$event.stopPropagation()" role="dialog" aria-modal="true" aria-labelledby="ef-title">
        ...
        <div class="drawer-header">
          <div class="drawer-title-group">
            ...
            <div>
              <h2 class="drawer-title" id="ef-title">Edit Farmer</h2>
              <p class="drawer-subtitle">{{ farmer().fullName }}</p>
            </div>
          </div>
        ...
        <form class="drawer-body" [formGroup]="form" (ngSubmit)="onSubmit()">
          ...
          <div class="field-row">
            <div class="field-group">
              <label class="field-label required" for="ef-firstName">First Name</label>
              <input id="ef-firstName" class="field-input" type="text" formControlName="firstName" [class.invalid]="invalid('firstName')">
              @if (invalid('firstName')) { <span class="field-error">First name is required</span> }
            </div>
            <div class="field-group">
              <label class="field-label required" for="ef-lastName">Last Name</label>
              <input id="ef-lastName" class="field-input" type="text" formControlName="lastName" [class.invalid]="invalid('lastName')">
          ...
          <div class="field-row">
            <div class="field-group">
              <label class="field-label required" for="ef-phone">Phone</label>
              <input id="ef-phone" class="field-input" type="tel" formControlName="phone" [class.invalid]="invalid('phone')">
              @if (invalid('phone')) { <span class="field-error">Phone is required</span> }
            </div>
            <div class="field-group">
              <label class="field-label" for="ef-email">Email <span class="field-optional">(optional)</span></label>
              <input id="ef-email" class="field-input" type="email" formControlName="email" [class.invalid]="invalid('email')">
              @if (invalid('email')) { <span class="field-error">Enter a valid email</span> }
            </div>
          </div>

          <div class="field-group">
            <label class="field-label required" for="ef-nationalId">National ID</label>
            <input id="ef-nationalId" class="field-input" type="text" formControlName="nationalId" [class.invalid]="invalid('nationalId')">
          ...
          <div class="field-row">
            <div class="field-group">
              <label class="field-label required" for="ef-region">Region</label>
              <select id="ef-region" class="field-input" formControlName="region" [class.invalid]="invalid('region')">
            ...
            <div class="field-group">
              <label class="field-label required" for="ef-district">District</label>
              <input id="ef-district" class="field-input" type="text" formControlName="district" [class.invalid]="invalid('district')">
...
export class EditFarmerModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.selectedCrops = Array.isArray(f.cropTypes) ? [...f.cropTypes] : [];

    // Load LBCs and pre-select current
    this.lbcsLoading.set(true);
    this.lbcService.list({ limit: 200 }).subscribe({
      next: (res) => {
        const options = res.data.map(l => ({ id: l.id, name: l.name, code: l.code }));
        this.lbcs.set(options);
        const current = options.find(l => l.id === f.lbcId);
        if (current) this.selectedLbc.set(current);
        this.lbcsLoading.set(false);
      },
      error: () => this.lbcsLoading.set(false),
    });

    // Load agents and pre-select current
    this.agentsLoading.set(true);
    this.agentService.list({ limit: 200 }).subscribe({
      next: (res) => {
        const options = res.data.map(a => ({ id: a.id, fullName: a.fullName, agentCode: a.agentCode }));
        this.agents.set(options);
        const current = options.find(a => a.id === f.agentId);
        if (current) this.selectedAgent.set(current);
  ...
  onSubmit(): void {
    ...
    const payload: Partial<CreateFarmerRequest> = {
    ...
    this.store.updateOne(this.farmer().id, payload, {
